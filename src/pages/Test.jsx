...
function Test() {
  ...
  const handleKeyDown = (event) => {
    if (event.key === 'Enter' && searchValue && !options.some(option => option.label === searchValue)) {
      const newOption = {
        label: searchValue,
        value: searchValue,
      };
      ...
      <div className="w-25">
        <Select
          ...
          dropdownRender={(menu) => (
            <>
              {menu}
              {searchValue && !options.some(option => option.label === searchValue) && (
                <div style={{ padding: '8px', cursor: 'pointer' }} onClick={() => handleKeyDown({ key: 'Enter' })}>
